...
$(document).ready(function(){
	...
	$('#botonCancelar').click(function(){
		window.location='precargaFormularioFactura.html?accionFactura='+$('#accionFactura').val();
	});
	if($('#accionFacturaDetalle').val()!="CONSULTAR"){
		
		$('#botonGuardar').click(function(){
			document.forms[0].submit();		
		});
		
		$('#botonGuardarSalir').click(function(){
...
//			}
		...
		$('#botonBuscarListaPrecios').click(function(){
			...
			if(codigoConceptoText!=null && codigoConceptoText!=undefined && codigoConceptoText.length>0){ 
				...
				jAlert("Debe ingresar primero un concepto válido.","Lista de Precios");
			}
		});
	}
...
function getConceptoFacturableAlicuotaCallBack(sResponseText){
	if (sResponseText != 'null' && sResponseText != ""){
		...
		$("#listaPreciosCodigoLabel").html('');
		$("#listaPreciosCodigo").val($('#codigoListaDefecto').val());
		$("#listaPreciosCodigo").blur();
