...
<html>
	<head>
		...
		<title>
			<spring:message code="formularioElemento.tituloSinReferencia" htmlEscape="true"/>
		</title>
		...
		<script type="text/javascript" src="js/mavalos_consulta_elementoSinReferencia.js"></script>
	</head>
		...
		<div class="contextMenu" id="myMenu1">
	  	...
	  	<div id="contenedorGeneral">
			...
			<fieldset style="border: none; text-align: left; width: 97%;">
					...
					<br/>
					<form:form action="filtrarRequerimientoElementoSinReferencia.html" commandName="elementoBusqueda" method="post">
						...
						<fieldset>
								...
								<table class="busqueda" style="width: 100%; background-color: white;">
									...
									<td class="texto_ti"><input type="text" id="codigoContenedor"
										...
										<td class="texto_ti">
											<select id="estado"
												name="estado" size="1">
												<option label="En el Cliente" value="En el Cliente" <c:if test="${elementoBusqueda.estado == 'En el Cliente'}">
															selected="selected"
												...
												<!-- 	<option label="Seleccione un Estado" value="">Seleccione un Estado</option> -->
														...
														<option label="Creado" value="Creado" 
															<c:if test="${elementoBusqueda.estado == 'Creado'}">
																selected="selected"
															</c:if>
														<c:if test="${elementoBusqueda!=null 
															&& elementoBusqueda.tipoRequerimiento!=null 
															&& elementoBusqueda.tipoRequerimiento.retiro}" >
															style="display:none;"</c:if>>Creado</option>
														...
														<c:if test="${elementoBusqueda!=null 
															&& elementoBusqueda.tipoRequerimiento!=null 
															&& elementoBusqueda.tipoRequerimiento.retiro}" >
															style="display:none;"</c:if>>En Guarda</option>
														...
														<c:if test="${elementoBusqueda!=null 
															&& elementoBusqueda.tipoRequerimiento!=null 
															&& elementoBusqueda.tipoRequerimiento.retiro}" >
															style="display:none;"</c:if>>En Transito</option>
														<option label="En el Cliente" value="En el Cliente" <c:if test="${elementoBusqueda.estado == 'En el Cliente'}">
															selected="selected"
														</c:if>>En el Cliente</option>	
							...
							&nbsp;&nbsp;
							<button type="button" id="botonPopupTipoElemento"
								title="<spring:message code="textos.buscar" htmlEscape="true"/>">
								<img src="<%=request.getContextPath()%>/images/buscar.png">
									...
									<tr>
										...
										<td class="texto_ti">
											...
											<c:if test="${elementoBusqueda==null 
												|| elementoBusqueda.tipoRequerimiento==null 
												|| !elementoBusqueda.tipoRequerimiento.retiro}" >
												<spring:message code="formularioElemento.vacio" htmlEscape="true" />
											</c:if>
										</td>
										<td rowspan="2" style="vertical-align: bottom;" class="texto_ti">
								...
								<c:out value="${descripcion}" default="" /> </label>
										...
										<td class="texto_ti">
											<c:if test="${elementoBusqueda!=null 
												...
												value="<c:out value="${elementoBusqueda.codigoClasificacionDocumental}" default="" />"
												 />
												&nbsp;&nbsp;
												...
												<label id="codigoClasificacionDocumentalLabel"
												for="codigoClasificacionDocumental"><c:out value="${elementoBusqueda.clasificacionDocumental.nombre}"  default=""/>  </label>
											</c:if>
											<c:if test="${elementoBusqueda==null 
												|| elementoBusqueda.tipoRequerimiento==null 
												|| !elementoBusqueda.tipoRequerimiento.retiro}" >
					...
					<fieldset>
						<input type="hidden" id="requerimientoElementosSel" name="requerimientoElementosSel" value=""/>
						<input type="hidden" id="sinReferencias" name="sinReferencias" value="true" />
						<display:table name="elementos" id="elemento"  pagesize="${pagesize}" sort="external" 
							requestURI="mostrarRequerimientoElementoSinReferencia.html" partialList="true" size="${size}" export="true">
							<display:column class="hidden" headerClass="hidden" media="html">
			              	...
			              	<display:column class="celdaAlineadoCentrado" title="<input type='checkbox' id='checktodos' name='checktodos'/>">
							    <input type="checkbox" class='checklote' value="${elemento.id}"/>
			              	</display:column>		
			              	<display:column class="hidden" headerClass="hidden" media="html">
							...
							<c:choose>
						  	...
						  	<display:column sortable="false" class="celdaAlineadoCentrado" media="html">
