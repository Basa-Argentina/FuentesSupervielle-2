 ...
 *
 */
public class ClientesPorEmpresaServlet extends HttpServlet{
	private static final long serialVersionUID = 7014807021831556429L;
	private static Logger logger = Logger.getLogger(ClientesPorEmpresaServlet.class);
	...
	private ClienteEmp getObject(String codigo, String codigoEmpresa,Long clienteId,Boolean habilitado){
		ClienteEmp cliente = new ClienteEmp();
		cliente.setCodigo(codigo);
		ClienteEmpService service = new ClienteEmpServiceImp(HibernateControl.getInstance());
		cliente = service.getByCodigo(codigo, codigoEmpresa, getObject(clienteId),habilitado);		
		return cliente;
	}
