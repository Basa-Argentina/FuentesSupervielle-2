...
import com.security.accesoDatos.facturacion.interfaz.ClienteEmailService;
 ...
 */
@Component
public class ClienteEmailServiceImp extends GestorHibernate<EmailCliente>implements ClienteEmailService {
	private static Logger logger = Logger.getLogger(ClienteEmailServiceImp.class);

	@Autowired
	public ClienteEmailServiceImp(HibernateControl hibernateControl) {
		super(hibernateControl);
	}

	@Override
	public Class<EmailCliente> getClaseModelo() {
		return EmailCliente.class;
	}
	...
	@Override
	public List<EmailCliente> listarClienteEmailFiltradas(EmailCliente cliente) {
		Session session = null;
		List<EmailCliente> emailClientes = null;
		...
		try {
			...
			if (cliente != null) {
				...
				if (cliente.getNombreCliente() != null && !"".equals(cliente.getNombreCliente()))
					crit.add(Restrictions.ilike("nombreCliente", cliente.getNombreCliente() + "%"));
				if (cliente.getEmails() != null && !"".equals(cliente.getEmails()))
					crit.add(Restrictions.ilike("emails", cliente.getEmails() + "%"));
			...
			if (val != null && candidatos.size() > 0) {
				...
				while (it.hasNext()) {
					...
					for (String filtro : valores) {
						if (candidato.getRazonSocialONombreYApellido().toLowerCase()
	...
	public EmailCliente getByID(Long id) {
		...
		try {
			...
			String consulta = " SELECT DISTINCT ce FROM EmailCliente ce WHERE ce.idCliente = " + id.longValue()
					+ "";
