...
public class SucursalesServlet extends HttpServlet{
	...
	private static Logger logger = Logger.getLogger(SucursalesServlet.class);
	...
	public void doGet (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!"".equals(codigo))
			sucursal = getObject(codigo, Long.valueOf(clienteIdStr));	
		if(sucursal != null){
	...
	private Sucursal getObject(String codigo, Long clienteId){
		Sucursal sucursal  = new Sucursal();
		SucursalService service = new SucursalServiceImp(HibernateControl.getInstance());
		sucursal = service.getByCodigo(codigo, getObject(clienteId));		
		return sucursal;
	}
